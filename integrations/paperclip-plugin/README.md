# Knowledge Mesh — Paperclip plugin

Exposes the Knowledge Mesh — the shared human/agent memory — to
agents orchestrated by [Paperclip](https://github.com/paperclipai/paperclip).
A thin connector over the Knowledge API: all logic, retrieval quality
...
- Repo→service mapping is pure string parsing of the PR URL — **no GitHub
  access**; Knowledge Mesh resolves the slug fuzzily (`order-handler-service` →
...
> agent's toolkit, and Knowledge Mesh ships one (`apps/mcp-server`). Pick the plugin
...
| Key | Default | Notes |
| --- | --- | --- |
| `apiUrl` | `http://127.0.0.1:3333` | Base URL of the Knowledge API |
...
correctly blocks private addresses for `ctx.http`, but a local-first Knowledge Mesh
...
paperclipai plugin data knowledge-mesh health --payload-json '{}'
```
